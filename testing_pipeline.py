...
@app.cell
def _(KuzuDatabaseManager, mo, pd, run_graph_rag):
    db_name = "nobel.kuzu"
    ...
    for i, question in enumerate(questions):
        with mo.status.spinner(title=f"Running Graph RAG on question {i}..."):

            # IMPORTANT: wrap `question` in a LIST
            results = run_graph_rag([question], db_manager)[0]
            result = results[0]
            latencie = results[1]
...
def _():
    ...
    #     for line in f:
    #         if line.lstrip().startswith("{'cache_retrieval':"):
    #             latency_lines.append(line.rstrip())
    return
...
def _(answers, latencies, pd, queries, questions):
    ...
    df_out.to_csv("data/test_results_with_caching2.csv", index=False)
    return
...
def _():
    ...
    # df_plot = pd.DataFrame(latency_lines)
...
def _(re):
    class RuleBasedProcessor():
        ...
        def get_label(self, item, query):
            ...
            pattern = rf"\(\s*{item_escaped}:([A-Za-z_][A-Za-z0-9_]*)\)"
...
def _(
    ...
    class GraphRAG(dspy.Module):
        ...
        async def aforward(self, db_manager: KuzuDatabaseManager, question: str, input_schema: str, _i: int):
            final_query, final_context = self.run_query(db_manager, question, input_schema, _i)
            if final_context is None:
                print("Empty results obtained from the graph database. Please retry with a different question.")
                return {}
            else:
    ...
    def run_graph_rag(questions: list[str], db_manager: KuzuDatabaseManager) -> list[Any]:
        ...
        for question in questions:
            ...
            results.append((response, rag.pipeline_latencies.copy()))
